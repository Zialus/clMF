...
void clmf(SparseMatrix& R, MatData& W_c, MatData& H_c, TestData &T, parameter& param, char filename[]) {
    ...
    CL_CHECK(clSetKernelArg(updateWOverH_kernel, 4, sizeof(VALUE_TYPE), &param.lambda));
    CL_CHECK(clSetKernelArg(updateWOverH_kernel, 5, sizeof(unsigned), &k));
    CL_CHECK(clSetKernelArg(updateWOverH_kernel, 6, sizeof(cl_mem), &WBuffer));
    CL_CHECK(clSetKernelArg(updateWOverH_kernel, 7, sizeof(cl_mem), &HBuffer));
